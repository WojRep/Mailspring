...
export interface ActunaMailWindowSettings {
  frame?: boolean;
  title?: string;
  width?: number;
  height?: number;
...
export default class ActunaMailWindow extends EventEmitter {
  static includeShellLoadTime = true;

  public windowType: string;
  public browserWindow: BrowserWindow & {
    loadSettings?: ActunaMailWindowSettings;
  ...
  constructor(settings: ActunaMailWindowSettings = {}) {
    super();
    ...
    if (process.platform === 'linux') {
      if (!WindowIconPath) {
        WindowIconPath = path.resolve('/usr', 'share', 'pixmaps', 'actunamail.png');
        if (!fs.existsSync(WindowIconPath)) {
          WindowIconPath = path.resolve(this.resourcePath, 'static', 'images', 'actunamail.png');
        }
      }
    ...
    if (ActunaMailWindow.includeShellLoadTime && !this.isSpec) {
      ActunaMailWindow.includeShellLoadTime = false;
    ...
    // user asked for: ActunaMail running silently in the background.
    if (isWaylandSession()) {
  ...
  handleEvents() {
    ...
    this.browserWindow.on('close', (event) => {
      ...
      // This check against false prevents that ActunaMail is closed when configuring the first mail account
    ...
    this.browserWindow.on('unresponsive', () => {
      ...
      const chosen = dialog.showMessageBoxSync(this.browserWindow, {
        ...
        message: 'ActunaMail is not responding',
    ...
    this.browserWindow.webContents.on('render-process-gone', (event, details) => {
      ...
      if (this.neverClose) {
        ...
        const chosen = dialog.showMessageBoxSync({
          ...
          message: 'ActunaMail has crashed',
