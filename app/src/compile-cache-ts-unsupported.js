...
function showDialog(filePath) {
  ...
  dialog.showErrorBox(
    ...
    `ActunaMail no longer ships with a TypeScript compiler to recompile plugins on the fly. ` +
...
['.ts', '.tsx', '.jsx', '.es6'].forEach(extension => {
  Object.defineProperty(require.extensions, extension, {
    ...
    value: (module, filePath) => {
      showDialog(filePath);
      throw new Error(
        `Cannot load ${filePath}: ActunaMail no longer ships with a TypeScript compiler. ` +
