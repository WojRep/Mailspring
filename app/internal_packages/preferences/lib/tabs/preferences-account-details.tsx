...
> {
  ...
  _onShowErrorDetails = async () => {
    ...
    try {
      ...
      const result = [
        `ActunaMail Version: ${AppEnv.getVersion()}`,
    ...
    switch (account.syncState) {
      case Account.SYNC_STATE_AUTH_FAILED:
        return this._renderErrorDetail(
          localized(
            `ActunaMail can no longer authenticate with %@. The password or authentication may have changed.`,
      ...
      case Account.SYNC_STATE_ERROR:
        return this._renderErrorDetail(
          localized(
            `ActunaMail encountered errors syncing this account. Crash reports have been sent to the ActunaMail team and we'll work to fix these errors in the next release.`
          ),
          [
            { text: localized('Reconnect'), action: this._onReconnect },
            { text: localized('Error Details...'), action: this._onShowErrorDetails },
