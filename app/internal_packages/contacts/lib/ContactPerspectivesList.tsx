    ...
    for (const group of groups) {
      ...
      items.push({
        ...
        onDrop: (item, { dataTransfer }) => {
          const data = JSON.parse(dataTransfer.getData('actunamail-contacts-data'));
        ...
        shouldAcceptDrop: (item, { dataTransfer }) => {
          if (!dataTransfer.types.includes('actunamail-contacts-data')) {
            return false;
          }
          ...
          const accountsType = dataTransfer.types.find((t) => t.startsWith('actunamail-accounts='));
...
const ContactsPerspectivesWithData: React.FunctionComponent<ContactsPerspectivesProps> = ({
  ...
  <ScrollRegion style={{ flex: 1 }} className="contacts-perspective-list">
    <section className="outline-view actunamail-outline-view" style={{ paddingTop: 15 }}>
      <OutlineViewItem
        item={{
          id: 'bla',
          name: 'All Contacts',
