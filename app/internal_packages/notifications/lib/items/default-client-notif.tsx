...
> {
      ...
      if (err) {
        if (err.message && err.message.includes('xdg-mime') && err.message.includes('not found')) {
          require('@electron/remote').dialog.showMessageBox({
            ...
            detail: localized(
              'ActunaMail could not find the xdg-mime utility. Please install the xdg-utils package using your system package manager (e.g. apt install xdg-utils) and try again from Preferences > General.'
            ),
          });
        } else {
  ...
  render() {
    ...
    return (
      <Notification
        title={localized('Would you like to make ActunaMail your default mail client?')}
        priority="1"
