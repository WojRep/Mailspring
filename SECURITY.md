...
- **PRO license file** — when the optional Actuna Engine PRO AI tier is licensed (#145+), `<engine-config>/license.json` holds a vendor-signed, offline token that includes the licensee's **own** email (the license is bound to it), plus an order reference and purchase date. No name, device id, or account id. It is **local-only and never transmitted**; verification, the email binding, and a 7-day liveness check are fully offline (zero egress). It contains the user's own address — a datum they already hold. See repo root `analysis/25-license-email-binding-memo.md`.
